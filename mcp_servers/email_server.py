...
def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
    ...
    return {
        "uid": uid,
        "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.helix.local>",
        "subject": subject,
...
def _fixture_account_rows() -> list[dict]:
    ...
    for row in _fixture_email_rows(owner or None):
        email_addr = row.get("account_email") or owner or "fixture@fixtures.helix.local"
        if email_addr not in owners:
            owners.append(email_addr)
    if not owners:
        owners = [owner or "fixture@fixtures.helix.local"]
    return [
        {
            "id": "fixture-email",
    ...
    try:
        ...
        conn.execute("""
            ...
            (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr,
             attachments, send_at, created_at, status, account_id, helix_kind, owner)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'agent_draft', ?, ?, ?)
        """, (
            pending_id,
    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email_reply so the user can review and send from H E L I X. "
            ...
            if result.get("pending"):
                return [TextContent(
                    ...
                    text=(
                        ...
                        "Nothing has been sent yet. Review and approve it in H E L I X before delivery."
                    ),
                )]
