...
document.addEventListener('helix:email-filter-tag', (e) => {
...
try { window.__helixGetActiveEmailContext = _getActiveEmailContext; } catch (_) {}
...
window.addEventListener('helix-reminder-channel-changed', (e) => {
...
export function openEmailLibrary(opts = {}) {
  ...
  if (tagsToggle) {
    ...
    tagsToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('helix.email.showTags', state._libShowTags ? '1' : '0');
      tagsToggle.classList.toggle('active', !!state._libShowTags);
      tagsToggle.setAttribute('aria-pressed', String(!!state._libShowTags));
      _renderGrid();
      document.dispatchEvent(new CustomEvent('helix:email-tags-toggle', { detail: { show: state._libShowTags } }));
    });
  }
