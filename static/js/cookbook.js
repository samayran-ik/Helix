  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. H E L I X will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="display:none;">'
       + 'Don\'t see a model? '
       + '<a href="https://github.com/pewdiepie-archdaemon/HELIX/discussions/1962" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;position:relative;top:-1px;">'
       + 'Request it →'
...
const SHARED_STATE_LEADER_KEY = 'helix-cookbook-shared-state-leader';
...
function _foregroundChatBusy() {
  try {
    return !!window.__helixChatBusy || Date.now() < (window.__helixChatBusyUntil || 0);
  } catch (_) {
    return false;
  }
}
