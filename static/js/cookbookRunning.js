...
function _winSessionStopTreePs(task) {
  ...
  return host
    ...
    : `${stopTree}; $p = Get-Content (Join-Path $env:TEMP 'helix-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p -match '^\\d+$') { Stop-Tree ([int]$p) }; Remove-Item (Join-Path $env:TEMP 'helix-tmux\\${sid}.*') -Force -ErrorAction SilentlyContinue`;
}
...
const BG_LEADER_KEY = 'helix-cookbook-bg-leader';
