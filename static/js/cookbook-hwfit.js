...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'H E L I X Cookbook hardware diagnostics',
      ...
      '',
      'Useful checks:',
      'docker compose exec helix nvidia-smi -L',
      'docker compose exec helix cat /proc/meminfo | head',
      'docker compose exec helix python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
