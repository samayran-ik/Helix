...
function _openEmailTagFilter(tag) {
  ...
  setTimeout(() => {
    document.dispatchEvent(new CustomEvent('helix:email-filter-tag', { detail: { tag: normalized } }));
  }, 0);
}
...
let _showEmailTags = localStorage.getItem('helix.email.showTags') !== '0';

export function init(documentModule) {
  _docModule = documentModule;
  _bindEvents();
  document.addEventListener('helix:email-tags-toggle', (e) => {
...
function _bindEvents() {
  ...
  // Delay the lightweight unread badge check so opening H E L I X doesn't
