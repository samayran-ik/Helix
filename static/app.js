// ============================================
// H E L I X UI — Main Application Orchestrator
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__helixSetChatMode = setMode;
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message H E L I X...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__helixSetChatMode === 'function') {
          window.__helixSetChatMode('chat');
        } else {
        ...
        if (typeof window.__helixSetChatMode === 'function') {
          window.__helixSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
...
function startHelixApp() {
  ...
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__helixChatBusyUntil = Math.max(window.__helixChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
