...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] helix doc minimal memory facts=%s", len(facts))
    return {
        "role": "user",
        "content": (
            "Saved user memory facts from H E L I X Brain. These are the same "
...
def _minimal_helix_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the H E L I X document LoRA.
    ...
    if stream_create:
        system = (
            "You are H E L I X. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are H E L I X. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, H E L I X will answer Done."
        )
...
async def stream_agent_loop(
    ...
    _ody_doc_finetune_mode = (
        (model or "").lower().startswith("helix-qwen3")
        and (
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        ...
        logger.info("[agent-intent] helix doc finetune tool clamp=%s", sorted(_relevant_tools))
    ...
    if _ody_doc_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_helix_doc_messages(
            messages,
        ...
        mcp_schemas = []
        logger.info(
            "[agent-intent] helix doc minimal prompt active active_doc=%s stream_create=%s messages=%s",
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] helix doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] helix doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _doc_stream_create_completed:
            ...
            logger.info("[agent] helix doc stream-create completed after one create_document")
            break
        ...
        if _ody_doc_tool_completed:
            if not full_response.strip() or full_response.strip().startswith("```"):
                full_response = "Done."
                yield 'data: ' + json.dumps({"delta": "Done."}) + '\n\n'
            logger.info("[agent] helix doc tool completed after one textual tool block")
            break
