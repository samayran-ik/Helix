...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    ...
    1. Tool calls: H E L I X carries `function.arguments` as a JSON *string*.
    ...
    2. Images (issue #4723): H E L I X carries multimodal user content as an
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/HELIX")
...
# value returns HTTP 400. H E L I X does not currently send the `thinking` mode
...
# HELIX_MISTRAL_REASONING_EFFORT (e.g. set to "medium" for cheaper chat).
_MISTRAL_REASONING_EFFORT = os.getenv("HELIX_MISTRAL_REASONING_EFFORT", "high")
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip HELIX-only metadata before sending messages to providers.
...
async def stream_llm(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Effort level is configurable via HELIX_MISTRAL_REASONING_EFFORT
