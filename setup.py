...
def create_default_admin():
    ...
    try:
        ...
        if username and password:
            ...
            if username in RESERVED_USERNAMES:
                print(f"  [error] HELIX_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] HELIX_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
...
def main():
    ...
    # Load .env so pre-seeded HELIX_ADMIN_USER / HELIX_ADMIN_PASSWORD (and
