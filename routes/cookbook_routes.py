...
_HF_TOKEN_STATUS_SNIPPET = (
    ...
    'Add one in H E L I X Cookbook -> Settings -> HuggingFace Token."; '
    'fi'
)
        ...
        if is_windows and remote:
            ...
            if not remote:
                ...
                if local_windows:
                    ...
                    # user PATH entries from the already-running H E L I X process.
            ...
            if ("llama_cpp" in req.cmd or "llama-server" in req.cmd) and not local_windows_llama_cmd:
                ...
                runner_lines.append('  [ -r ~/.config/helix-llama-cpp-env ] && . ~/.config/helix-llama-cpp-env')
                ...
                runner_lines.append('      echo "[helix] NVIDIA detected but installed llama-cpp-python is CPU-only — reinstalling with CUDA wheel index for GPU offload..."')
                runner_lines.append('      python3 -m pip install --user --break-system-packages --force-reinstall --no-cache-dir "llama-cpp-python[server]" --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cu124 2>&1 | tail -8 || echo "[helix] WARNING: CUDA wheel reinstall failed — Python server will stay CPU-only (slow). Manual fix: pip install --user --force-reinstall \'llama-cpp-python[server]\' --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cu124"')
                runner_lines.append('      if python3 -c "import llama_cpp; import sys; sys.exit(0 if llama_cpp.llama_supports_gpu_offload() else 1)" 2>/dev/null; then')
                runner_lines.append('        echo "[helix] llama-cpp-python now supports GPU offload."')
                runner_lines.append('      fi')
                runner_lines.append('    fi')
                runner_lines.append('  fi')
                ...
                runner_lines.append('# Auto-generated by H E L I X Cookbook: a `llama-server` lookalike')
                ...
                runner_lines.append('    echo "[helix] Created llama-server shim → python -m llama_cpp.server (no native binary needed)"')
                runner_lines.append('  fi')
                ...
                runner_lines.append('  HELIX_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
                runner_lines.append(f"HELIX_SERVE_CMD='{_bash_squote(req.cmd)}'")
                runner_lines.append('if [ -z "$HELIX_PREFLIGHT_EXIT" ]; then')
                runner_lines.append('  HELIX_VLLM_HELP_CMD="$(python3 - "$HELIX_SERVE_CMD" <<\'PY\'')
                ...
                runner_lines.append(')"')
                runner_lines.append('  HELIX_VLLM_SUPPORTS_SWAP=0')
                runner_lines.append('  if eval "$HELIX_VLLM_HELP_CMD" 2>&1 | grep -q -- "--swap-space"; then HELIX_VLLM_SUPPORTS_SWAP=1; fi')
                runner_lines.append('fi')
                runner_lines.append('if [ -z "$HELIX_PREFLIGHT_EXIT" ] && [ "${HELIX_VLLM_SUPPORTS_SWAP:-0}" = "1" ] && ! printf "%s" "$HELIX_SERVE_CMD" | grep -q -- "--swap-space"; then')
                runner_lines.append('  echo "[helix] Setting vLLM --swap-space 0 so the runtime does not reserve CPU swap per GPU."')
                runner_lines.append('  HELIX_SERVE_CMD="${HELIX_SERVE_CMD} --swap-space 0"')
                runner_lines.append('fi')
                runner_lines.append('if [ -z "$HELIX_PREFLIGHT_EXIT" ] && [ "${HELIX_VLLM_SUPPORTS_SWAP:-0}" != "1" ]; then')
                runner_lines.append('  if printf "%s" "$HELIX_SERVE_CMD" | grep -q -- "--swap-space"; then')
                runner_lines.append('    echo "[helix] vLLM serve does not expose --swap-space; removing the flag and patching the runtime default to 0."')
                runner_lines.append('    HELIX_SERVE_CMD="$(python3 - "$HELIX_SERVE_CMD" <<\'PY\'')
                runner_lines.append('import shlex, sys')
                runner_lines.append('parts = shlex.split(sys.argv[1])')
                ...
                runner_lines.append('def _helix_swap0(cls):')
                ...
                runner_lines.append('_helix_swap0(EngineArgs)')
                runner_lines.append('_helix_swap0(AsyncEngineArgs)')
                runner_lines.append('try:')
                ...
                runner_lines.append('def _helix_create_engine_config(self, *args, **kwargs):')
                runner_lines.append('    self.swap_space = 0')
                runner_lines.append('    return _orig_create_engine_config(self, *args, **kwargs)')
                runner_lines.append('EngineArgs.create_engine_config = _helix_create_engine_config')
                runner_lines.append('AsyncEngineArgs.create_engine_config = _helix_create_engine_config')
                ...
                runner_lines.append('PY')
                runner_lines.append(')"')
                runner_lines.append('  echo "[helix] Patched vLLM internal swap_space default to 0 for this runtime."')
                runner_lines.append('fi')
            ...
            if not handled_ollama_serve and not handled_ollama_sidecar_probe:
                ...
                if "vllm serve" in req.cmd:
                    runner_lines.append('eval "$HELIX_SERVE_CMD"')
                else:
                    runner_lines.append(req.cmd)
                if local_windows:
