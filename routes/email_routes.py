...
def _apply_helix_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-HELIX-Origin"] = HELIX_MAIL_ORIGIN
    if kind:
        msg["X-HELIX-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-HELIX-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-HELIX-Kind "reminder" SUBJECT "Reminder (H E L I X):"{from_clause})',
                )
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-HELIX-Kind {_search_quote("reminder")})'))
