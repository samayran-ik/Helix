...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('HELIX_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for HELIX-managed `pip install --user`
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _helix_have_prebuilt=1
    ...
    runner_lines.append('    _helix_have_prebuilt=""')
    runner_lines.append('    _helix_arch="$(uname -m)"')
    runner_lines.append('    _helix_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_helix_arch" = "x86_64" ]; then')
    runner_lines.append('      _helix_pat=""')
    runner_lines.append('      _helix_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _helix_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _helix_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _helix_has_nv_inline; then')
    runner_lines.append('        _helix_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _helix_has_vkdev_inline && _helix_has_vk_inline; then')
    runner_lines.append('        _helix_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _helix_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _helix_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_helix_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_helix_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[helix] Found prebuilt llama-server: $_helix_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/helix/llama-cpp-prebuilt" && cd "$HOME/.cache/helix/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_helix_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _helix_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_helix_extracted" ]; then')
    runner_lines.append('          chmod +x "$_helix_extracted"')
    runner_lines.append('          ln -sf "$_helix_extracted" ~/bin/llama-server')
    runner_lines.append('          _helix_libdir="$(dirname "$_helix_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_helix_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/helix-llama-cpp-env')
    runner_lines.append('          _helix_have_prebuilt=1')
    runner_lines.append('          echo "[helix] Prebuilt llama-server installed at $_helix_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_helix_have_prebuilt" ] && echo "[helix] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_helix_prebuilt_url" ]; then')
    runner_lines.append('      echo "[helix] No matching prebuilt llama-server for this host (arch=$_helix_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_helix_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _helix_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _helix_has_nvidia_hw; then')
    ...
    runner_lines.append('    _helix_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[helix] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[helix] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[helix] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[helix] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _helix_apt_bootstrap')
    runner_lines.append('    _helix_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _helix_missing_build_deps="$_helix_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _helix_missing_build_deps="$_helix_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _helix_missing_build_deps="$_helix_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_helix_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_helix_missing_build_deps"')
    ...
    runner_lines.append('    _helix_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _helix_has_vulkan_device() {')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _helix_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _helix_has_vulkan_device && _helix_has_vulkan; then')
    runner_lines.append('      echo "[helix] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[helix] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[helix]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[helix]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _helix_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[helix] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[helix] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
