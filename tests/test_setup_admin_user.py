...
def test_main_loads_admin_password_from_env_file(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HELIX_ADMIN_USER", raising=False)
    monkeypatch.delenv("HELIX_ADMIN_PASSWORD", raising=False)
    (tmp_path / ".env").write_text(
        "HELIX_ADMIN_USER=presetuser\nHELIX_ADMIN_PASSWORD=fromenvfile12345\n",
        encoding="utf-8-sig",
    )
    ...
    monkeypatch.setenv("HELIX_SKIP_ADMIN_PROMPT", "1")
    ...
    try:
        ...
        # don't inherit them.
        os.environ.pop("HELIX_ADMIN_USER", None)
        os.environ.pop("HELIX_ADMIN_PASSWORD", None)
