"""Regression: `helix-calendar list` must select events that OVERLAP the
...
`helix-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "helix-calendar"
    loader = importlib.machinery.SourceFileLoader("helix_calendar_cli", str(path))
