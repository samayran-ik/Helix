...
def _compose_env_names(path: Path) -> set[str]:
    ...
    env = compose["services"]["helix"]["environment"]
...
def _upload_limit_env_names() -> set[str]:
    ...
    return set(re.findall(r'"(HELIX_[A-Z_]*BYTES)"', source)) | {
        "HELIX_CHAT_UPLOAD_MAX_BYTES"
    }
