...
if getattr(sys, 'frozen', False):
    ...
    def show_splash_instantly():
        ...
        try:
            ...
            tk.Label(splash_root, text="⛵ H E L I X", font=("Segoe UI", 22, "bold"), bg="#1a1c23", fg="#e06c75").pack(pady=(22, 2))
...
def create_tray_image():
    # Generate a beautiful 64x64 icon matching H E L I X brand red accent (#e06c75)
...
def setup_system_tray(url):
    try:
        ...
        menu = (
            pystray.MenuItem('Open H E L I X', lambda icon, item: on_open_browser(icon, item, url), default=True),
...
def initialize_ollama():
    """Initialize Ollama service before starting the app."""
    try:
        # Suppress stdout/stderr for cleaner output in frozen mode
        import io
        import logging
        
        # Set up logging for Ollama manager
        logging.basicConfig(
            level=logging.INFO,
            format='%(message)s'
        )
        ...
        # Try to ensure Ollama is ready (auto-install on Windows, pull model if needed)
        if manager.ensure_ollama_ready(auto_install=True):
            return True
        else:
            # Log warning but don't block startup - user can still use other LLM providers
            import logging
            logger = logging.getLogger(__name__)
            logger.warning("⚠ Ollama could not be fully initialized. You can still use other LLM providers.")
            return False
    
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"⚠ Ollama initialization error: {e}. Continuing without Ollama.")
        return False
...
if __name__ == "__main__":
    ...
    # Initialize Ollama before starting the app (runs on main thread to block until ready)
