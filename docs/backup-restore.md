...
H E L I X keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/helix-backup` tool
...
```bash
# Create a snapshot → backups/helix-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/helix-backup snapshot

# List existing snapshots (most recent first)
./scripts/helix-backup list

# Check a tarball's integrity without extracting it
./scripts/helix-backup verify backups/helix-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/helix-backup restore backups/helix-backup-20260101-120000.tar.gz --yes
```
...
./scripts/helix-backup snapshot --out /mnt/nas/helix-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/helix-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/helix && ./scripts/helix-backup snapshot --out "/mnt/nas/helix-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `helix-backup` therefore does not capture the Docker
