# H E L I X Setup Guide

This page keeps the detailed install, deployment, troubleshooting, and configuration notes out of the front README.

## Quick Start

> **Branch note:** `dev` is the default branch and contains the latest development changes, but it may be unstable. For the more stable curated branch, use [`main`](https://github.com/pewdiepie-archdaemon/HELIX/tree/main).
...
### Docker (recommended)
```bash
git clone https://github.com/HELIX.git
cd helix
...
does not mount `/var/run/docker.sock`. You can still connect H E L I X to
...
models through HELIX/llama.cpp, so Windows users with an existing Ollama
...
does **not** read `APP_BIND` / `HELIX_HOST` from `.env`, so editing `.env`
...
- **`AUTH_ENABLED=false` is ignored / you're still forced to log in (Windows).** If you edited `.env` in Notepad it may have saved a UTF-8 **BOM**, turning the first key into `﻿AUTH_ENABLED` so it is never matched. H E L I X loads `.env` with `encoding="utf-8-sig"` to tolerate a leading BOM, but the safe fix is to re-save `.env` as **UTF-8 without BOM** (VS Code: *Save with Encoding → UTF-8*).
...
- **Calendar/contacts (Radicale) won't sync.** Point H E L I X at the **full collection URL** with its trailing slash — e.g. `http://host:5232/<user>/<collection-id>/` — not just the server root. Radicale shows this address for each calendar/address book in its web UI.
...
1. Keep H E L I X on localhost, for example `127.0.0.1:7000`.
...
Cloudflare Access, Tailscale, Caddy, nginx, and Traefik can all fit this pattern; none are required by H E L I X. If your access layer reaches H E L I X on the same host, proxy to `http://127.0.0.1:7000` and keep `AUTH_ENABLED=true`, `LOCALHOST_BYPASS=false`, and `SECURE_COOKIES=true`.
