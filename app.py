...
try:
    ...
    # H E L I X is single-process by convention, so this is acceptable, but be aware that
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-HELIX-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-HELIX-Owner") or "").strip()
...
async def _startup_event():
    ...
    # Initialize Ollama on startup (non-blocking background task)
    async def _startup_ollama():
        """Initialize Ollama service in the background during startup."""
        try:
            from core.ollama_manager import OllamaManager
            manager = OllamaManager()
            # Run Ollama initialization in a thread to avoid blocking async startup
            result = await asyncio.to_thread(manager.ensure_ollama_ready, True, None)
            if result:
                logger.info("[startup] ✓ Ollama is ready")
            else:
                logger.warning("[startup] ⚠ Ollama could not be fully initialized (non-critical)")
        except Exception as e:
            logger.debug(f"[startup] Ollama initialization skipped: {e}")
    ...
    # Initialize Ollama in the background
    _startup_tasks.append(asyncio.create_task(_startup_ollama()))
    ...
    _keepalive_enabled = str(os.getenv("HELIX_MODEL_KEEPALIVE", "")).lower() in {"1", "true", "yes", "on"}
